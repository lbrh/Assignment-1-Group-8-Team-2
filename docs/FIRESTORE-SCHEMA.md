...
**Path:** `/notes/{noteId}`
**Access:** Owner-only (user can read/write their own notes; admins can read all)
...
**Creation:** Created via `frontend/src/features/notes/actions/notes.actions.ts`.
**Deletion:** Soft-delete using `deletedAt` field (not implemented yet — use if needed).
