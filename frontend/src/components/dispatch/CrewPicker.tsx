...
export function CrewPicker() {
  ...
  }, [incidentId, loadCrews]);

  const available = incident
    ? crews
  ...
  function submit(e: FormEvent) {
    e.preventDefault();
    if (!incident || picked.length === 0) return;
    dispatchCrews(incident.id, picked);
      ...
      {incident ? (
        <form onSubmit={submit} className="crew-picker__body">
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <p className="caption">
              {incident.ref} · nearest first, straight-line distance from each crew&apos;s station.
            </p>
          </div>
          ...
          {available.length === 0 ? (
            <p style={{ font: "400 var(--text-sm)/1.5 var(--font-plex-sans)", color: "var(--fg-2)" }}>
              {crews.length === 0
                ? "Loading crews…"
                : "Every crew is out. Recall one from another incident first."}
            </p>
          ) : (
              ...
              {available.map(({ crew, km }) => (
                <label key={crew.id} className="crew-option">
                  <input
                    ...
                    onChange={(e) =>
                      setPicked((p) => (e.target.checked ? [...p, crew.id] : p.filter((id) => id !== crew.id)))
                    }
                  />
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "var(--space-2)", flexWrap: "wrap" }}>
            ...
            <Button type="submit" variant="primary" disabled={picked.length === 0}>
              {picked.length > 1 ? `Dispatch ${picked.length} crews` : "Dispatch crew"}
            </Button>
          </div>
        </form>
      ) : null}
